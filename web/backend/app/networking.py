...
def _parse_ifconfig_output(ifconfig_output: str) -> List[LocalInterfaceIPv4]:
    ...
    for raw_line in ifconfig_output.splitlines():
        ...
        # Example: en0: flags=...
        if line and not line.startswith("\t") and line.endswith(":") is False and ":" in line:
            # A heuristic to detect start of an interface stanza.
        ...
        # Example: inet 192.168.1.10 netmask 0xffffff00 broadcast 192.168.1.255
        if stripped.startswith("inet ") and "netmask" in stripped:
            # Tokenize; inet line usually follows `inet <ip> netmask <maskhex> ...`
...
def get_local_ipv4_interfaces() -> List[LocalInterfaceIPv4]:
    ...
    # Prefer the most specific (largest prefixlen) subnet.
...
def normalize_subnet_input(value: str) -> str:
    ...
    Accepts either:
    - CIDR: `10.0.0.0/24`
    - IP + prefix/netmask: `10.0.0.187/24` or `10.0.0.187/255.255.255.0`
    ...
    if default_iface:
        # Refuse to guess silently; guide the user.
        raise ValueError(
            f"IP {v} does not match the detected local networks. "
            f"Please provide CIDR (e.g. 10.0.0.0/24) or use the detected subnet ({default_iface.cidr})."
        )
